...
=======
        ...
        if (dirTimer > dirInterval) //同上，改变扭曲方向
        {
            ...
            ChangeHighlight(currentHighlight-nextDir);   //同时改变轨道的高亮
        }
...
=======
        ...
        foreach (GameObject e in elements)
        {
            ...
            originalMat[0] = e.transform.FindChild(currentHighlight.ToString()).GetComponent<MeshRenderer>().material;
            originalMat[1] = e.transform.FindChild(((currentHighlight + 1) % 10).ToString()).GetComponent<MeshRenderer>().material;
